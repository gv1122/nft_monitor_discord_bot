...
const webhook = JSON.parse(fs.readFileSync("./config.json", "utf8")).icyFloor;

function sendWebhook(data: any) {
	let allData = "";

	data.forEach((item: any) => {
		allData += `[${item.name}](https://opensea.io/collection/${item.slug}) - ${item.floor}Ξ\n`;
	});

	axios.post(webhook, {
		"embeds": [
			{
				"title": "Top Lowest Sale Price - Last hour",
				"description": allData,
...
export async function floorTrending() {
	axios.request({
		method: "POST",
		...
		data: {
			query: `
				query TrendingCollections {
					contracts(orderBy: FLOOR, orderDirection: DESC) {
					  edges {
						node {
						  address
						  ... on ERC721Contract {
		...
		const data = res.data.data.contracts.edges.slice(0, 5).map(({node}) => {
			return {
				...
				floor: node.stats.floor
			};
		});

		sendWebhook(data);
	}).catch(async() => {
		console.log("rate limit on floor trending");
		await sleep(2000);
		floorTrending();
	});
}