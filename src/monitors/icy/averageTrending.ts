...
const webhook = JSON.parse(fs.readFileSync("./config.json", "utf8")).icyAverage;

function sendWebhook(data: any) {
	let allData = "";

	data.forEach((item: any) => {
		allData += `[${item.name}](https://opensea.io/collection/${item.slug}) - ${item.average}Ξ\n`;
	});

	axios.post(webhook, {
		"embeds": [
			{
				"title": "Top Average Sale Price - Last hour",
				"description": allData,
...
export async function averageTrending() {
	axios.request({
		method: "POST",
		...
		data: {
			query: `
				query TrendingCollections {
					contracts(orderBy: AVERAGE, orderDirection: DESC) {
					  edges {
						node {
						  address
						  ... on ERC721Contract {
		...
		const data = res.data.data.contracts.edges.slice(0, 5).map(({node}) => {
			return {
				...
				average: node.stats.average
			};
		});

		sendWebhook(data);
	}).catch(async () => {
		console.log("rate limit in average trending");
		await sleep(2000);
		averageTrending();
	});
}