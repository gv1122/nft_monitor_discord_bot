...
const webhook = JSON.parse(fs.readFileSync("./config.json", "utf8")).icySales;

function sendWebhook(data: any) {
	let allData = "";

	data.forEach((item: any) => {
		allData += `[${item.name}](https://opensea.io/collection/${item.slug}) - ${item.sales} Sales\n`;
	});

	axios.post(webhook, {
		"embeds": [
			{
				"title": "Top Sales - Last hour",
				"description": allData,
...
export async function salesTrending() {
	axios.request({
		method: "POST",
